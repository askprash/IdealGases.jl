...
- **`:seeded` (default — exact)**: the table provides only the *seed* for
  ...
  iterations, typically 1–2 polish steps). The answer satisfies the strict
  inversion contract wherever the literal NASA-9 polynomials have a unique
  inverse. Targets outside the tabulated range bypass the table entirely and
  use the plain cold-start `FrozenGas` solve — never extrapolation.

...
# interval. The tiny published-data seam exception is handled below.
const NEWTON_RTOL = 1e-12
const NEWTON_MAXITER = 30

# The published NASA-9 coefficient records are intended to join at 1000 K,
# but their finite printed precision leaves a tiny enthalpy discontinuity for
# some species. Keep the stored/evaluated coefficients literal; only the
# inverse recognizes a target between the two one-sided limits as the
...
enthalpies there, no unique literal-polynomial inverse exists, so this function
...
implicit-function-theorem rules from the package extension, never
differentiation of the Newton loop. Pure and zero-allocation.
"""

...
@testset "FrozenGas pure core" begin
    ...
    @testset "T_from_h resolves NASA-9 seam targets at 1000 K" begin
        ...
        # between the two one-sided values has no unique literal-polynomial
        # inverse (upward jump) or two nearby roots (downward jump). The public
        # inverse uses 1000 K as the deterministic canonical answer in either
        # case, while retaining the 1e-12 Newton solve for every real root.
        for name in ("H2O", "CH4")
            ...
            # enthalpy is a seam endpoint and must round-trip without a
            # one-ULP Newton crossing into the low polynomial.
